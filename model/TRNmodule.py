...
from model.Resnet_reg import resnet50
from base import  BaseModel
import numpy as np

def return_relationset(num_frames, num_frames_relation):
    import itertools
    indice_all = list(itertools.combinations([i for i in range(num_frames)], num_frames_relation))

    indice_return=[]
    if num_frames_relation < 6:
        for indice in indice_all:
            total_dist = 0
            indice= list(indice)
            for i in range(1, len(indice)):
                total_dist += (indice[i] - indice[i - 1])
            if total_dist > num_frames_relation + 1 :
                indice_return.append(indice)
        return indice_return[::len(indice_return)//3+1]
    else:
        return indice_all[::len(indice_return)//3+1]

class TRN_module(BaseModel):
    

    def __init__(self, img_feature_dim, num_frames, num_class):
        super(TRN_module, self).__init__()
...
class TRN_withclassifier(BaseModel):
    # relation module in multi-scale with a classifier at the end
    def __init__(self, img_feature_dim, num_frames, num_class):
        super(TRN_withclassifier, self).__init__()
        self.subsample_num = 3 # how many relations selected to sum up
        self.img_feature_dim = img_feature_dim
