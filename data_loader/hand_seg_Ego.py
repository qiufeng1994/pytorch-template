...
def ego_dataset_preprocess():
    """
    TODO: turn polygons to mask
    """
    import scipy
    from PIL import ImageDraw
    import cv2
    data = Path('/Data/egohands')
    dirs = (data / '_LABELLED_SAMPLES').glob('*')
    color = data / 'color'
    ...
    for num, dir in enumerate(dirs):
        ...
        imgs.sort()
        mat = scipy.io.loadmat(Path(dir).glob('*.mat')[0])
        
        for i in range(100):
            canvas = Image.new('L', (1280, 720), 0)
            for j in names:
                polygons = mat['polygons'][j][0][i].reshape(-1).tolist()
                if polygons:
                    ImageDraw.Draw(canvas).polygon(polygons, outline=1, fill=1)
            mask = np.array(canvas) * 255
            cv2.imwrite(data / 'hand_mask' / 'ego_{:05}.png'.format(num * 100 + i), mask.astype('uint8'))
            cv2.imwrite(data / 'color' / 'ego_{:05}.png'.format(num * 100 + i), cv2.imread(imgs[i]))
            print('{}/4800: saving to {}'.format(num * 100 + i, data / 'mask_hand' / 'ego_{:05}.png'))
